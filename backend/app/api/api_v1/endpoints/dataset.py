...
# from app.ml.fast_recommendation_engine import fast_engine  # Removed FAISS dependency
...
# Keep the original fast endpoint for backward compatibility
@router.post("/recommendations/fast/")
async def get_fast_recommendations(request: SmartAppraisalRequest):
    """
    🚀 REDIRECTED to optimized endpoint
    This endpoint now uses NumPy + scikit-learn instead of FAISS
    For backward compatibility - redirects to optimized implementation
    """
    try:
        # Redirect to the optimized implementation
        return await get_optimized_recommendations(request)
        
    except Exception as e:
        logging.error(f"❌ Fast recommendations failed: {e}")
